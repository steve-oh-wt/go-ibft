...
var _ core.Backend = &mockBackend{}
...
func newMockCluster(
	...
	for index := 0; index < int(numNodes); index++ {
		...
		nodes[index] = core.NewIBFT(logger, backend, transport)
...
type mockCluster struct {
	nodes []*core.IBFT      // references to the nodes in the cluster
...
func (m *mockCluster) runSequence(height uint64) {
	...
	for nodeIndex, node := range m.nodes {
		...
		go func(ctx context.Context, node *core.IBFT) {
...
func (m *mockCluster) setBaseTimeout(timeout time.Duration) {
	for _, node := range m.nodes {
		node.SetBaseRoundTimeout(timeout)
	}
}
